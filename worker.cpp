...
public:
	...
	void event_handle(int , msgpack::object obj, msgpack::zone* ){
		...
		case OP::GET_RANGE:{ // op, attr_name, range, org_addres
			...
			if(range.get_begin() < myrange.get_begin() || myrange.get_end() < range.get_end()){ // if out of my range
				int counter = 0;
				...
				while(i != hub.end()){
					if(range.contain(i->first) && i->second != address(settings.myip,settings.myport)){
						attr_range common;
						range.get_common_range(i->first, &common);
			...
			if(!fwd.empty()){
				...
				while(it != fwd.end()){
					const MERDY::get_range get_range(OP::GET_RANGE,name,identifier,it->second,address(settings.myip,settings.myport)); 
					tuple_send(get_range,it->first);
					++it;
				}
				newfwd->cnt--;
				if(newfwd->cnt == 0){
					mp::sync< std::unordered_multimap<mer_fwd_id,mer_get_fwd*,mer_fwd_id_hash> >::ref mer_range_fwd_r(mer_range_fwd);
					const MERDY::ok_get_range ok_get_range(OP::OK_GET_RANGE, name, identifier, newfwd->toSend);
					tuple_send(ok_get_range, newfwd->org);
		...
		case OP::GET_ATTR:{
			...
			std::unordered_multimap<std::string, mercury_instance>::const_iterator it = mer_node_r->find(name);
			...
			while(req != requires.end()){
				int ok=0;
				it = it_bak;
				while(it != mer_node_r->end() && it->first == name){
					if(it->second.get_range().contain(*req)){
						ok = 1;
						const std::map<attr, uint64_t>& kvp = it->second.kvp;
		...
		case OP::OK_TELLME_RANGE:{
			...
			MERDY::ok_tellme_range ok_tellme_range(obj);
		...
		case OP::NG_TELLME_RANGE:{
			...
			MERDY::ng_tellme_range ng_tellme_range(obj);
