...
public:
    hash_lock():set(){}
    mutex& get_mutex(int target){
        return set[target % locknum];
    }
};

struct lock_set : public singleton<lock_set>{
private:
    enum lock_entries{
        send_fwd_locks = 32,
    };
public:
    lock_set(){}
    hash_lock<send_fwd_locks> send_fwd_lock;
};
...
inline void print_packed_size(const obj& target){
    msgpack::sbuffer buf;
    msgpack::pack(buf,target);
    fprintf(stderr,"packed_size:%lu\n",buf.size());
}
...
public:
    ...
    void event_handle(int , msgpack::object obj, mp::shared_ptr<msgpack::zone> z){
        ...
        case OP::SET_DY:{// op, key, value
            ...
            const msgpack::object& value = set_dy.get<2>();
            const address& org = set_dy.get<3>();
			
            DEBUG(std::cerr << "key[" << key << "] value[" << value << "]"<< std::endl);
        ...
        case OP::SET_COORDINATE:{// op, key, value, address
            ...
            const msgpack::object& value = set_coordinate.get<2>();
            ...
            value_vclock vcvalue;
            {
                ...
                if(result == key_value_r->end()){
                    vcvalue.update_increment(value,address(settings.myip,settings.myport));
                    key_value_r->insert(std::pair<uint64_t, value_vclock>(key,vcvalue));
                }else{
                    result->second.update_increment(value,address(settings.myip,settings.myport));
                    vcvalue = result->second;
                }
            }
            ++it;
            if(it == dy_hash.end()){
        ...
        case OP::GET_DY:{
            ...
            }
            {
                ...
                */
                mutex::scoped_lock lock(lock_set::instance().send_fwd_lock.get_mutex(key));
                tc_send_fwd.insert(std::pair<uint64_t, get_fwd_t>(key,get_fwd_t(org)));
            }
        ...
        case OP::SEND_DY:{
            ...
            if(ans == key_value_r->end()){
                const MERDY::notfound_dy* notfound_dy = z->allocate<MERDY::notfound_dy>(OP::NOTFOUND_DY, key, address(settings.myip,settings.myport));
                tuple_send_async(notfound_dy, org, &sockets,z);
                DEBUG_OUT("not found");
            }else{
                const MERDY::found_dy* found_dy = z->allocate<MERDY::found_dy>(OP::FOUND_DY, key, ans->second, address(settings.myip,settings.myport));
                {
                    msgpack::sbuffer testbuf;
                    msgpack::pack(&testbuf,found_dy->a2);

                    int len = testbuf.size();
                    const char* ptr = testbuf.data();

                    fprintf(stderr,"size:%d ", len);
                    while(len > 0){
                        fprintf(stderr,"%02X", (*ptr) & 0x000000ff);
                        ++ptr;
                        --len;
        ...
        case OP::FOUND_DY:{ // op, key, vcvalue, address
            ...
            // lock
            mutex::scoped_lock bucket_lock(lock_set::instance().send_fwd_lock.get_mutex(key));
            ...
            bool result = it->second.add(value);
            tc_send_fwd.update(it);
            if(result == false){ // old data found -> read repair
                const MERDY::put_dy* put_dy = z->allocate<MERDY::put_dy>(OP::PUT_DY, key, it->second.get_vcvalue()[0], address(settings.myip,settings.myport));
            ...
            if(it->second.count_eq(DY::READ)){
                ...
                if(ans == key_value_r->end()){
                    key_value_r->insert(std::pair<uint64_t, value_vclock>(it->first,it->second.get_vcvalue()[0]));
                }
                const MERDY::ok_get_dy* ok_get_dy = z->allocate<MERDY::ok_get_dy>(OP::OK_GET_DY,key,it->second.get_vcvalue(),address(settings.myip,settings.myport));
                tuple_send_async(ok_get_dy, it->second.org, &sockets, z);
                DEBUG(it->second.dump());
        ...
        case OP::NOTFOUND_DY:{ 
            ...
            if(it != send_fwd.unsafe_ref().end()){
                // read repair
                if(!it->second.get_vcvalue().empty()){
                    const MERDY::put_dy* put_dy = z->allocate<MERDY::put_dy>(OP::PUT_DY, key, it->second.get_vcvalue()[0], address(settings.myip,settings.myport));
                    tuple_send_async(put_dy, org, &sockets, z);
                }
            }
        ...
        case OP::GET_RANGE:{ // op, attr_name, range, org_addres
            ...
            mutex::scoped_lock lock(dbg_mut);
            ...
            while(true) {
                if(m_pac.execute()) {
                    ...
                    try{
                        ...
                        fprintf(stderr,"convertion error: %s\n",e.what());
